...
import type { DeflateBlock, DeflateItem, HuffmanTreeEntry, LengthTableEntry, DistanceTableEntry } from './types';

export class DeflateParser {
  private static readonly LENGTH_TABLE: LengthTableEntry[] = [
    { code: 257, extraBits: 0, length: 3 }, { code: 258, extraBits: 0, length: 4 }, { code: 259, extraBits: 0, length: 5 }, { code: 260, extraBits: 0, length: 6 }, { code: 261, extraBits: 0, length: 7 },
    { code: 262, extraBits: 0, length: 8 }, { code: 263, extraBits: 0, length: 9 }, { code: 264, extraBits: 0, length: 10 }, { code: 265, extraBits: 1, length: 11 }, { code: 266, extraBits: 1, length: 13 },
    { code: 267, extraBits: 1, length: 15 }, { code: 268, extraBits: 1, length: 17 }, { code: 269, extraBits: 2, length: 19 }, { code: 270, extraBits: 2, length: 23 }, { code: 271, extraBits: 2, length: 27 },
    { code: 272, extraBits: 2, length: 31 }, { code: 273, extraBits: 3, length: 35 }, { code: 274, extraBits: 3, length: 43 }, { code: 275, extraBits: 3, length: 51 }, { code: 276, extraBits: 3, length: 59 },
    { code: 277, extraBits: 4, length: 67 }, { code: 278, extraBits: 4, length: 83 }, { code: 279, extraBits: 4, length: 99 }, { code: 280, extraBits: 4, length: 115 }, { code: 281, extraBits: 5, length: 131 },
    { code: 282, extraBits: 5, length: 163 }, { code: 283, extraBits: 5, length: 195 }, { code: 284, extraBits: 5, length: 227 }, { code: 285, extraBits: 0, length: 258 }
  ];

  private static readonly DISTANCE_TABLE: DistanceTableEntry[] = [
    { code: 0, extraBits: 0, distance: 1 }, { code: 1, extraBits: 0, distance: 2 }, { code: 2, extraBits: 0, distance: 3 }, { code: 3, extraBits: 0, distance: 4 }, { code: 4, extraBits: 1, distance: 5 },
    { code: 5, extraBits: 1, distance: 7 }, { code: 6, extraBits: 2, distance: 9 }, { code: 7, extraBits: 2, distance: 13 }, { code: 8, extraBits: 3, distance: 17 }, { code: 9, extraBits: 3, distance: 25 },
    { code: 10, extraBits: 4, distance: 33 }, { code: 11, extraBits: 4, distance: 49 }, { code: 12, extraBits: 5, distance: 65 }, { code: 13, extraBits: 5, distance: 97 }, { code: 14, extraBits: 6, distance: 129 },
    { code: 15, extraBits: 6, distance: 193 }, { code: 16, extraBits: 7, distance: 257 }, { code: 17, extraBits: 7, distance: 385 }, { code: 18, extraBits: 8, distance: 513 }, { code: 19, extraBits: 8, distance: 769 },
    { code: 20, extraBits: 9, distance: 1025 }, { code: 21, extraBits: 9, distance: 1537 }, { code: 22, extraBits: 10, distance: 2049 }, { code: 23, extraBits: 10, distance: 3073 },
    { code: 24, extraBits: 11, distance: 4097 }, { code: 25, extraBits: 11, distance: 6145 }, { code: 26, extraBits: 12, distance: 8193 }, { code: 27, extraBits: 12, distance: 12289 },
    { code: 28, extraBits: 13, distance: 16385 }, { code: 29, extraBits: 13, distance: 24577 }
  ];

  private static readonly CODE_LENGTH_ORDER = [16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15];
  ...
  private parseFixedHuffmanBlock(reader: BitReader): DeflateBlock {
    ...
    try {
      ...
      content += `Parse error: ${error instanceof Error ? error.message : 'Unknown error'}\n`;
    }
  ...
  private parseDynamicHuffmanBlock(reader: BitReader): DeflateBlock {
    ...
    try {
      ...
      for (let i = 0; i < hclen; i++) {
        codeLengths[DeflateParser.CODE_LENGTH_ORDER[i]] = reader.readBits(3);
      }
      ...
      while (allCodeLengths.length < totalCodes && attempts < 1000) {
        ...
        try {
          ...
          throw new Error(`Error decoding code length at position ${allCodeLengths.length}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }
      
      if (attempts >= 1000) {
        throw new Error('Hit attempt limit, stopping code length decoding');
      }
  ...
  private buildHuffmanTree(codeLengths: number[], maxSymbol: number): Record<number, HuffmanTreeEntry> {
    ...
    for (let i = 0; i <= maxSymbol; i++) {
      ...
      if (len !== 0) {
        ...
        if (isNaN(codeValue) || codeValue === undefined) {
          throw new Error(`Invalid code value for length ${len}, nextCode[${len}] = ${nextCode[len]}`);
        }
  ...
  private decodeSymbol(reader: BitReader, tree: Record<number, HuffmanTreeEntry>): { symbol: number; bitStart: number; bitEnd: number } {
    let code = 0;
    let bits = 0;
    const startPos = reader.getPosition();
    ...
    throw new Error(`Invalid Huffman code: ${code} at position ${reader.getPosition().byte}.${reader.getPosition().bit}`);
  }
  ...
  private readDistanceCodeFixed(reader: BitReader, fixedDistanceCodes: Record<number, HuffmanTreeEntry>): { symbol: number; bitStart: number; bitEnd: number } {
    let code = 0;
    let bits = 0;
    const startPos = reader.getPosition();
